    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="space-y-6">
          ...
          {ticket.raSlaDeadline && ticket.channelType === "RECLAMEAQUI" && !["RESOLVED", "CLOSED"].includes(ticket.status) && (
            <Card className={(() => {
              ...
              if (days <= 0) return "border-red-500 bg-red-50 dark:bg-red-950";
              if (days <= 2) return "border-red-300 bg-red-50/50 dark:bg-red-950/50";
              if (days <= 5) return "border-yellow-300 bg-yellow-50 dark:bg-yellow-950";
              return "";
            })()}>
              <CardHeader>
                <CardTitle className="text-lg flex items-center gap-2">
              ...
              <CardContent>
                {(() => {
                  ...
                  let text = `RA: ${days} dias úteis restantes`;
                  if (days <= 0) { badgeColor = "bg-black text-white"; text = days === 0 ? "RA: Vence hoje!" : `RA: Expirado há ${Math.abs(days)} dia(s)`; }
                  else if (days <= 2) { badgeColor = "bg-red-100 text-red-800"; text = `RA: ${days} dia(s) restante(s) ⚠️`; }
                  else if (days <= 5) { badgeColor = "bg-yellow-100 text-yellow-800"; text = `RA: ${days} dias restantes`; }
                  return (
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="text-sm">Prazo público (10 dias úteis)</span>
                        <span className={`px-2 py-1 rounded text-xs font-bold ${badgeColor}`}>{text}</span>
                      </div>
                      <p className="text-xs text-muted-foreground">
                        Vencimento: {dl.toLocaleDateString("pt-BR")}
                      </p>
                    </div>
                  );
                })()}
