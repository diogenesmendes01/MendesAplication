...
describe("feedback-capture", () => {
  ...
  describe("captureFeedback", () => {
    it("captures positive on APPROVED", async () => { const id = await captureFeedback(base, "APPROVED"); expect(id).toBe("fb-1"); expect(mockCreate.mock.calls[0][0].data.type).toBe("positive"); expect(mockCreate.mock.calls[0][0].data.diff ?? null).toBeNull(); });
